var loader;
var ADMIN_STATS_API_URL = `${base_url}/api/admin/stats`;
...
window.onload = function () {
    ...
    $.ajax({
        ...
        if (xhr.status === 200) {
            document.querySelector("#total_users").innerText = response.total_users;
            document.querySelector("#total_analyses").innerText = response.total_analyses;
            document.querySelector("#unread_alerts").innerText = response.unread_alerts;
            show_emotion_chart(response.emotion_distribution)
            show_depression_chart(response.depression_distribution);
            return;
        } else {
